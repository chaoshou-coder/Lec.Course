...
> 告诉 agent 你想学什么。它扒知识依赖、推理学习路径、按知识点制课、独立验收。
...
1. **发现(DISCOVER)** —— 深挖具体目的(做什么/给谁/为什么/何时/成功标准) + 学员画像 + 验收标准 + 你选择并发度
...
└── skills/
    └── course-agent/              # skill 入口
        ...
        └── state-machine.md       # 状态机架构(6 态 + 转移规则)
```

## 产出结构(示例)

框架产出的课程结构如下(以 Git 版本控制为例):
...
└── output/                        # 流程产物
    ...
    ├── learning-plan.json         # PLAN 产出
    └── qa-report.json             # QA 产出
```

**特点:**
- 每个知识点(#### 节)包含完整的 8 步趁热打铁循环(痛点→类比→解释→ASCII→代码→逐行解剖→常见错误→练习)
- 每个知识点末尾有独立的学员代码区 + 参考答案(不是只在每天末尾)
