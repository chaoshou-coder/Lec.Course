...
### 6.1 必须深挖具体目的(why_before_what)

**核心问题:** 用户说"我想学 X"时,背后有一个**具体问题**要解决。DISCOVER 必须挖出这个问题,否则产出的课程会"正确但无用"。

**深挖五问(按顺序):**

| 顺序 | 问题 | 目的 | 示例(用户说"我想学 HTML") |
|------|------|------|--------------------------|
| 1 | **你要做什么东西?** | 从抽象→具体 | "一个个人作品集网站" |
| 2 | **做给谁看/用?** | 明确受众 | "给招聘方看,找工作用" |
| 3 | **解决你什么痛点?** | 找到真实动机 | "现在简历没项目可写,面试被刷" |
| 4 | **什么时候要完成?** | 时间约束 | "3 周后投简历" |
| 5 | **做好了长什么样?** | 可视化成功标准 | "有 4 个页面,能在线访问,手机也能看" |

**规则:**
- 必须按顺序问,跳问会遗漏关键信息
- 第 1 问如果用户已经给出具体答案(如"博客系统"),跳到第 2 问
- 第 3 问(痛点)是最关键的——**如果答不出来,说明需求不真实,需要回到第 1 问重新收敛**
- 所有答案写入 `requirements.json` 的 `specific_purpose` 字段

**反例(当前 skill 的输出):**
```json
{
  "target_capability": "独立完成一个个人作品集网站的 HTML 编码与上线部署"
}
```
→ 没有受众、没有痛点、没有时间约束、没有成功标准。课程"正确但无用"。
...
```json
{
  "target_capability": "独立完成一个个人作品集网站的 HTML 编码与上线部署",
  "specific_purpose": {
    "what": "个人作品集网站(首页/关于我/项目展示/联系表单)",
    "who": "招聘方(前端/全栈岗位面试官)",
    "why": "简历没项目可写,面试被刷;需要 3 周内有作品可展示",
    "deadline": "3 周后开始投简历",
    "success_looks_like": "有 4 个页面,能在线访问,手机也能看,代码在 GitHub"
  }
}
```

### 6.2 必须定义学员画像(learner_persona)
...
### 6.4 acceptance_criteria 必须覆盖 ≥2 个认知层级
...
### 6.6 target_capability 必须收敛(v1.1 R4)
...
### 6.7 理解层 criteria 不用"背诵"(v1.1 R4)
...
### 6.8 必须写 success_scenarios(v1.1 R4)
